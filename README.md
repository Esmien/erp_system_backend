...
1. Регистрацию/авторизацию пользователей (`JWT`), добавление токена в blacklist в Redis при выходе с TTL равным
   оставшемуся времени жизни токена (для защиты от OOM)
2. RBAC/ABAC систему управления доступом к ресурсам по ролям и дополнительному контексту через политики доступа (`JSONB` в `PostgreSQL`) с кэшированием в Redis
...
Здесь нет абстракций ради абстракций или излишнего переусложнения. Каждое архитектурное решение принято после нескольких итераций рефакторинга.
...
**Роли по умолчанию:**
* `admin` (полные права)
* `manager` (частичные права)
* `user` (ограниченные права на создание)
