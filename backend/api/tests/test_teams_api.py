async def test_get_team_info_by_id_success(client, response_get_team_by_id):
    response = await client.get("/v1/teams/1")
...
async def test_get_team_info_by_id_not_exists(client):
    response = await client.get("/v1/teams/999")
...
async def test_create_team_success(
    ...
    response = await client.post("/v1/teams/", json=request_body_for_create_team)
...
async def test_create_team_duplicate_name(
    ...
    response = await client.post("/v1/teams/", json=request_duplicate)
...
async def test_join_success(
    ...
    response = await client.post("/v1/teams/join", json=join_team_right_code_request)
...
async def test_join_already_in_team(
    ...
    await client.post("/v1/teams/join", json=join_team_right_code_request)
    response = await client.post("/v1/teams/join", json=join_team_right_code_request)
...
async def test_join_wrong_code(
    ...
    response = await client.post("/v1/teams/join", json=join_team_wrong_code_request)
